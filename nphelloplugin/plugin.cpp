...
static NPIdentifier sfuncTestMethod1_id;
...
ScriptablePluginObjectBase::_InvokeDefault(NPObject *npobj,
                                           ...
                                           NPVariant *result)
{
	OutputDebugString(L"_InvokeDefault");
...
ScriptablePluginObject::Invoke(NPIdentifier name, const NPVariant *args,
                               uint32_t argCount, NPVariant *result)
{
	if (name == sfuncTestMethod1_id)
	{
		return TestMethod1(args, argCount, result);
	}
	// Sample code
	else if (name == sFoo_id) {
		OutputDebugString(_T("foo called!"));

		char buffer[] = "foo return val";
	...
	//STRINGZ_TO_NPVARIANT(strdup("foo return val"), *result); // 리턴할 때 _strdup를 사용하면 안 됨. NPN_MemAlloc을 써야함.
                                      ...
                                      NPVariant *result)
{
  ...
  char buffer[] = "default method return val";
  uint32_t len = strlen(buffer) + 1;
  char *copy = (char*)NPN_MemAlloc(len);
  //STRINGZ_TO_NPVARIANT(copy, *result);
  ...
  m_pScriptableObject(NULL)
{
  ...
  sfuncTestMethod1_id = NPN_GetStringIdentifier("TestMethod1");
  ...
  if (NPVARIANT_IS_INT32(rval) && NPVARIANT_TO_INT32(rval) == 4) {
    OutputDebugString(L"Default function call SUCCEEDED!");
  } else {
    OutputDebugString(L"Default function call FAILED!");
  }
